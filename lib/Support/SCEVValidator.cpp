...
public:
  ...
  virtual ValidatorResult visitUnknown(const SCEVUnknown *Expr) {
...
public:
  NonAffSCEVValidator(const Region *R, ScalarEvolution &SE, const Value *BaseAddress)
  ...
  virtual class ValidatorResult visitUnknown(const SCEVUnknown *Expr) {
    ...
    if (Expr->isSizeOf(AllocTy) ||
        Expr->isAlignOf(AllocTy) ||
...
getParamsInNonAffineExpr(const Region *R, const SCEV *Expr, ScalarEvolution &SE,
                      const Value *BaseAddress) {
  if (isa<SCEVCouldNotCompute>(Expr))
    return std::vector<const SCEV *>();
